...
extern void log_append_supplemental_log (THREAD_ENTRY * thread_p, SUPPLEMENT_REC_TYPE rec_type, int length,
					 const void *data);
extern int log_append_supplemental_lsa (THREAD_ENTRY * thread_p, SUPPLEMENT_REC_TYPE rec_type, OID * classoid, LOG_LSA * undo_lsa, LOG_LSA * redo_lsa);
...
extern int xlog_reader_get_log_refined_info (THREAD_ENTRY * thread_p, LOG_LSA * start_lsa, int *total_length,
					     int *num_log_info);
extern int xlog_reader_set_configuration (THREAD_ENTRY * thread_p, int max_log_item, int timeout, int all_in_cond,
					  char **user, int num_user, uint64_t * classid, int num_class);
